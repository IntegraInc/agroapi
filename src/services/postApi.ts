...
export async function postApi({ options }: { options: optionsType }) {
 ...
 try {
  ...
  const updateRange = `Senha_Cliente!A${rowIndex}:N${rowIndex}`; // Ajuste conforme necessário para incluir múltiplas colunas
