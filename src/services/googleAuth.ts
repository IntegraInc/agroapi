...
export async function googleAuth({ options }: { options: optionsType }) {
 ...
 const range = "Senha_Cliente!C:D"; // Ajuste a range conforme necessário
 try {
  ...
  const cpfIndex = rows[0].indexOf("CPF");
  ...
  const updateRange = `Senha_Cliente!D${rowIndex}`; // Ajuste conforme necessário
